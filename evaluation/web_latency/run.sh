...
max_client_num=($(jq -r '.max_client_number' configs/web_latency.json))
...
for dp_interval_peer2 in "${dp_intervals_peer2[@]}"; do
  ...
  ./helpers/run_testbed.sh --hostname_peer1 $peer1_ssh_host --username_peer1 $peer1_ssh_username --hostname_peer2 $peer2_ssh_host --username_peer2 $peer2_ssh_username --netshaper_dir_peer1 $peer1_netshaper_dir --netshaper_dir_peer2 $peer2_netshaper_dir --iter_num $iter_num --results_dir $experiment_results_dir --max_client_num $max_client_num --request_rate $request_rate
