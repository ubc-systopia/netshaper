 ...
 *     │                 ┌──────────────┐         ┌────────────┐      ┌──────────┐      │                │           │
 *     │                 │              │         │            │      │ POST_SFT │      └────────────────┘           │
 *     │   L4 TRAFFIC    │  SFT ACTION  │         │ MARK STATE │      │          │              ▲                    │
 *     │   + FRAG        │  JUMP TO     │         │ IN SFT     │      │ CHECK    │              │                    │
 *     │  (when enabled) │  TABLE WITH  ├────────►│            ├─────►│ VALID FID├──────────────┘                    │
 *     │ ┌─────────────► │  PREDEFINED  │         │            │      │ &&       │                                   │
 *     │ │               │  ZONE        │         │            │      │ VALID    │                                   │
 *     │ │               │              │         │            │      │   STATE  │                                   │
 *     │ │               └──────────────┘         └────────────┘      └┬─────────┘                                   │
 ...
 *┌────┼─┴┐                                                            │  TRAFFIC      ┌─────────┐                   │
 *│    │  │                                                            └───────────────►         │              ┌────┼──┐
 *│ PORT  │               NON L4 TRAFFIC + FRAG (when disabled)                        │  HAIRPIN│              │    │  │
 *│ RX │  ├────────────────────────────────────────────────────────────────────────────►         │              │  PORT │
 *│    │  │                                                                            │  QUEUE  ├─────────────►│  TX│  │
 *└────┼──┘                                                                            │         │              │    │  │
 *     │                                                                               └─────────┘              └────┼──┘
...
enum PRE_SFT_GROUP_PRIORITY {
	FRAG_PRIORITY = 0,	/* Priority for rules that should handle fragmented traffic */
	JUMP_TO_SFT_PRIORITY = 1,		/* Priority for rules that should jump to SFT */
...
struct application_rules {
	...
	struct rte_flow *forward_fragments[SFT_PORTS_NUM * SFT_IP_VERSIONS_NUM * SFT_IP_PROTOCOLS_NUM];
	/* Rules to hairpin fragmented packets when fragmentation is not enabled */
	struct rte_flow *hairpin_fragments[SFT_PORTS_NUM * SFT_IP_VERSIONS_NUM * SFT_IP_PROTOCOLS_NUM];
};
 ...
 */
void print_offload_rules_counter(void);
