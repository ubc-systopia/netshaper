...
def dp_interval_vs_overhead_web_unidirectional(config: configlib.Config, filtered_data_list):
...
def dp_interval_vs_overhead_web_biidirectional(config: configlib.Config, filtered_data_list):
    alphas = [1 + x / 10.0 for x in range(1, 100)] + list(range(12, 64))
    ...
    for index, filtered_data in enumerate(filtered_data_list):
        ...
        webpage_nums_list = config.webpage_nums_lis
        with tqdm(total=len(webpage_nums_list)*len(noise_multipliers_s_to_c)) as pbar:
...
def number_of_traces_vs_overhead_web(config: configlib.Config, filtered_data_list):
    if config.communication_type == "bidirectional":
        return dp_interval_vs_overhead_web_biidirectional(config, filtered_data_list)
    elif config.communication_type == "unidirectional":
        return dp_interval_vs_overhead_web_unidirectional(config, filtered_data_list)
    else: 
