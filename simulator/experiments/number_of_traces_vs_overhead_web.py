...
def number_of_traces_vs_overhead_web(config: configlib.Config, filtered_data):
    ...
    average_aggregated_overhead_pacer, std_aggregated_overhead_pacer = get_pacer_web_overhead(filtered_data)
    ...
    with tqdm(total=len(webpage_nums_list)*len(noise_multipliers)) as pbar:
        for noise_multiplier in noise_multipliers:
            for webpage_num in webpage_nums_list:
                max_dp_decision = webpage_num * constant_rate_max
                filtered_data_parallelized = get_parallel_dataframe(filtered_data, webpage_num, config.max_webpage_per_experiment)
